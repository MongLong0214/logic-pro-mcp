...
@Test func testServerCatalogSnapshotIncludesCommercialSurface() {
    ...
    #expect(snapshot.startupBanner == "Starting logic-pro-mcp v3.12.0 — 10 tools, \(snapshot.resourceURIs.count) resources, 4 channels")
}

@Test func testServerCatalogStartupBannerUsesProvidedChannelCount() {
    let banner = ServerCatalog.startupBanner(channelCount: 7)
    #expect(banner == "Starting logic-pro-mcp v3.12.0 — 10 tools, \(ResourceProvider.resources.count) resources, 7 channels")
}
...
@Test func testLogicProServerCompositionSnapshotMatchesExpectedOrder() async {
    ...
    #expect(snapshot.startupBanner == "Starting logic-pro-mcp v3.12.0 — 10 tools, \(ResourceProvider.resources.count) resources, 7 channels")
}
