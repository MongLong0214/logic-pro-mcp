...
inputs_digest() {
    ...
    [ -n "$list" ] || return 2
    # A FAILED MEASUREMENT MUST NOT BECOME A MEASUREMENT. Every reading below was once written as
    # `printf '...' "$(cmd)"`, where the substitution fails, printf succeeds with an empty field,
    # and the empty field is hashed — so a file the tool could not read contributed a digest of
    # nothing and the binding still reported success. Reproduced 2026-09-08 in a scratch repo with a
    # `git` that fails only on `hash-object`: every file failed, stderr said so twice, and this
    # function returned a clean digest and exit 0. That is the exact defect this script exists to
    # prevent, in the script that exists to prevent it.
    #
    # `set -o pipefail` does not save this: the failure is inside a command substitution, not a
    # pipeline stage. Each reading is captured, checked, and turned into `return 2` (cannot tell).
    {
        printf '%s\n' "$list" | while IFS= read -r f; do
            [ -f "$f" ] || continue
            h=$(git hash-object -- "$f") || { echo "CANNOT-TELL(2): could not hash $f" >&2; exit 3; }
            [ -n "$h" ] || { echo "CANNOT-TELL(2): empty hash for $f" >&2; exit 3; }
            printf '%s  %s\n' "$h" "$f"
        done || exit 3
        ...
        # were committed.
        pins=$(git show HEAD:Package.resolved 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
        [ -n "$pins" ] || { echo "CANNOT-TELL(2): could not read committed Package.resolved" >&2; exit 3; }
        printf 'pins  %s\n' "$pins"
        swiftver=$(swift --version 2>&1 | head -1)
        [ -n "$swiftver" ] || { echo "CANNOT-TELL(2): swift --version produced nothing" >&2; exit 3; }
        printf 'swift  %s\n' "$swiftver"
        printf 'config  %s\n' "${LPM_BUILD_CONFIG:-release}"
    } > "$TMPINPUTS" || return 2
    # The subshell's `exit 3` lands here as a non-zero status; without this check the caller would
    # hash whatever partial output had been written before the failure.
    [ $? -eq 0 ] || return 2
    [ -s "$TMPINPUTS" ] || return 2
    shasum -a 256 < "$TMPINPUTS" | cut -d' ' -f1
}

case "${1:?usage: $0 emit|check [<binary>]}" in
  emit)
      [ -f "$BIN" ] || { echo "CANNOT-TELL(2): no binary at $BIN" >&2; exit 2; }
      # REFUSE A BINDING THE BUILD CANNOT HAVE PRODUCED. `emit` hashes the worktree and the
      # executable independently; nothing here observes the compile that would connect them, so
      # "build A, edit to B, emit" would happily bind binary A to source B. A review found that
      # 2026-09-08.
      #
      # mtime cannot prove freshness and this does not ask it to. It can prove STALENESS: an
      # executable older than an input was certainly not built from it. So the one direction mtime
      # is sound in is used as a refusal, and the other direction stays an unproven limit the
      # `limits` field of any citing record has to carry.
      NEWER=$(git ls-files -- Sources Package.swift 2>/dev/null \
              | while IFS= read -r f; do [ -f "$f" ] && [ "$f" -nt "$BIN" ] && printf '%s\n' "$f"; done \
              | head -3)
      if [ -n "$NEWER" ]; then
          echo "CANNOT-TELL(2): these inputs are newer than $BIN, so it was not built from them:" >&2
          printf '  %s\n' $NEWER >&2
          echo "  Rebuild, then emit. A binding minted here would be a false one." >&2
          exit 2
      fi
