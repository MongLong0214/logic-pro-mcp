...
def ledger_casefold_path() -> str:
    ...
    never invented.
    """
    return os.path.join(LEDGER_DIR, "casefold.tsv")


def ledger_strings(repo: str = REPO) -> set[str]:
    """Every string the label ledger asks the corpus about: each label's canonical and variants."""
    with open(os.path.join(repo, "docs", "locale", "ui-labels.json"), encoding="utf-8") as handle:
        labels = json.load(handle).get("labels") or {}
    return {text for entry in labels.values()
            for text in [entry.get("canonical")] + list(entry.get("variants") or []) if text}
...
def load_ledger_casefold() -> set[tuple[str, str, str]]:
    path = ledger_casefold_path()
    if not os.path.exists(path):
        raise CanonError(
            f"{path} does not exist, so no label string is known to ship up to case. Run "
            f"Scripts/logic_canon.py build on a machine with Logic.")
    stamp = os.stat(path)
...
def write_ledger_casefold(rows) -> int:
...
def ships_up_to_case(source: str, locale: str, text: str) -> bool:
    ...
    `mixerNamedElement`'s canonical is `mixer`, a German Logic ships the row capitalised, and every
...
#: What `presence` answers. SHIPS comes only from a pinned full comparison; ABSENT from the 32-bit
#: set, which cannot produce a false one, or from a pinned comparison that found the prefix match
#: was a collision; UNCONFIRMED is a prefix match nobody compared.
SHIPS, ABSENT, UNCONFIRMED = "ships", "absent", "unconfirmed"
#: The same for the near-miss fold: does a shipped value fold, under `fold_for_near_miss`, to it.
NEAR, FAR = "near", "far"


def ledger_presence_path() -> str:
    """`source <TAB> locale <TAB> exact <TAB> near <TAB> "text"`: a string comparison, pinned.

    #992. The absence sets are 32-bit digest prefixes. They answer ABSENT soundly, because every
    value the corpus holds put its own prefix in the set. They cannot answer PRESENT: a string
    Apple does not ship can share a prefix with one it does. Eight guard sites read
    `not is_absent(...)` as proof that Logic ships a string. The ones that refuse on presence
    refused a legitimate change on a collision, with nothing offline that could rebut it; the ones
    that credit presence passed a string Logic does not ship.

    So a string whose prefix matches is compared as a STRING, on a machine with Logic, and the
    answer is pinned here. `exact` is `ships` when the string is a value of the corpus after
    `normalize` (the values the absence set is written over) and `absent` otherwise; `near` is
    `near` when a value folds to it under `fold_for_near_miss` and `far` otherwise. A row exists
    only where a 32-bit set matched -- everywhere else the set is already the answer.

    Rows are written by `confirm` for the strings it is given and re-derived by every `build` for
    the strings already here, so a string Apple stops shipping changes verdict with the rebuild
    that saw it go. The strings are the repository's own questions -- a literal, a reading, a
    canonical -- so nothing is disclosed that the tree does not already quote.
    """
    return os.path.join(LEDGER_DIR, "presence.tsv")


_PRESENCE_CACHE: dict = {}


def load_presence_ledger() -> dict:
    """`{(source, locale, text): (exact, near)}`. A missing file pins nothing: every prefix match
    then reads UNCONFIRMED, which each guard treats as not proven rather than as shipped."""
    path = ledger_presence_path()
    if not os.path.exists(path):
        return {}
    stamp = os.stat(path)
    key = (path, stamp.st_size, stamp.st_mtime_ns)
    ...
    with open(path, "r", encoding="utf-8") as handle:
        for number, line in enumerate(handle, 1):
            ...
            parts = line.split("\t")
            if len(parts) != 5:
                raise CanonError(f"{path}:{number}: expected 5 tab-separated fields, "
                                 f"got {len(parts)}")
            source, locale, exact, near, quoted = parts
            if exact not in (SHIPS, ABSENT) or near not in (NEAR, FAR):
                raise CanonError(f"{path}:{number}: verdicts must be {SHIPS}|{ABSENT} and "
                                 f"{NEAR}|{FAR}, not {exact!r} and {near!r}")
            try:
...
def write_ledger_presence(rows: dict) -> int:
    ...
    with open(ledger_presence_path(), "w", encoding="utf-8") as handle:
        ...
        handle.write("# Generated by Scripts/logic_canon.py confirm and build: strings whose 32-bit\n"
                     "# prefix is in an absence set, compared as strings against the corpus.\n"
                     "# Do not hand-edit.\n")
        for (source, locale, text) in sorted(rows):
            exact, near = rows[(source, locale, text)]
            handle.write(f"{source}\t{locale}\t{exact}\t{near}\t"
                         f"{json.dumps(text, ensure_ascii=False)}\n")
    return len(rows)


def presence_rows(values_by_locale_by_source: dict, texts) -> dict:
    """The rows a full comparison pins for `texts`: one per corpus where a 32-bit set matches.
    ...
    for source, by_locale in sorted(values_by_locale_by_source.items()):
        for locale, values in sorted(by_locale.items()):
            ...
            for text in wanted:
                near_text = fold_for_near_miss(text)
                if _u32(text) not in prefixes and _u32(near_text) not in folded_prefixes:
                    continue
                rows[(source, locale, text)] = (SHIPS if text in values else ABSENT,
                                                NEAR if near_text in folded else FAR)
    return rows


def presence_counts(rows) -> dict:
    """Rows per source and locale, the shape `MANIFEST.json` declares them in."""
    return ledger_counts({(source, locale, text) for (source, locale, text) in rows})
...
def verify_presence_ledger(manifest: dict) -> list:
    """The presence ledger holds the rows `MANIFEST.json` counts, and each row agrees with the sets.

    Read for PRESENCE like the casefold ledger, so an added `ships` row is the forgery that
    matters; the declared count is the third edit it needs. And a row is written only where a
    32-bit set matched, so a `ships` row whose prefix is not in the exact set, or a `near` row
    whose fold is not in the folded set, was not taken from the corpus those sets describe.
    """
    declared = manifest.get("ledger_presence_entries")
    if not isinstance(declared, dict):
        return ["docs/canon/MANIFEST.json declares no `ledger_presence_entries`, so nothing checks "
                "that rows were not added to ledger/presence.tsv. Run Scripts/logic_canon.py "
                "confirm or build."]
    try:
        rows = load_presence_ledger()
    except CanonError as exc:
        return [f"ledger/presence.tsv: {exc}"]
    problems = []
    found = presence_counts(rows)
    for source in sorted(set(declared) | set(found)):
        want_by_locale = declared.get(source) or {}
        have_by_locale = found.get(source) or {}
        for locale in sorted(set(want_by_locale) | set(have_by_locale)):
            ...
            if want != have:
                stated = "declares no count" if want is None else f"declares {want}"
                problems.append(
                    f"ledger/presence.tsv holds {have} row(s) for {source}/{locale} and "
                    f"MANIFEST.json {stated}. An added row credits a string Apple does not ship.")
    for (source, locale, text), (exact, near) in sorted(rows.items()):
        try:
            if exact == SHIPS and is_absent(source, locale, text):
                problems.append(
                    f"ledger/presence.tsv says {text!r} ships in {source}/{locale} and no 32-bit "
                    f"prefix in absence/{source}.{locale}.u32 matches it.")
            if near == NEAR:
                ...
                if not (position < len(table) and table[position] == needle):
                    problems.append(
                        f"ledger/presence.tsv says a value folds to {text!r} in {source}/{locale} "
                        f"and no 32-bit prefix in the folded set matches it.")
        except CanonError as exc:
            problems.append(f"ledger/presence.tsv row {text!r} ({source}/{locale}): {exc}")
    return problems
...
def differs_only_by_decoration(source: str, locale: str, text: str) -> bool:
    ...
    Only a string that is ABSENT can be a near miss. It used to ask `is_absent`, so a string whose
    prefix merely collided read as shipped and its near miss was never looked for (#992). A pinned
...
def build(app: str, *, sources: list[str], refresh_citations: bool, repo: str = REPO) -> dict:
    ...
    # The presence ledger's strings, compared again (#992). Same keep-and-replace as above.
    pinned = load_presence_ledger()
    presence_kept = {row: verdict for row, verdict in pinned.items()
                     if row[0] not in values_by_locale_by_source}
    presence_kept.update(presence_rows(values_by_locale_by_source,
                                       {text for (_source, _locale, text) in pinned}))
    write_ledger_presence(presence_kept)
    manifest["ledger_presence_entries"] = presence_counts(presence_kept)
...
def confirm(app: str, texts) -> dict:
    """Compare `texts` as strings against the corpus the manifest pins, and pin the answers.

    Needs Logic. The 32-bit sets this settles were written over particular bytes, so this refuses
    unless the installed Logic is the pinned build AND every source's corpus digest is the pinned
    one: a comparison over other bytes would settle a question about a different corpus. Strings
    already in the ledger are compared again. Only `ledger/presence.tsv` and the manifest's count
    and artifact digests change.
    """
    manifest = load_manifest()
    here = app_build(app)
    if manifest.get("logic") != here:
        raise CanonError(f"this Logic is {here} and the pinned corpus is {manifest.get('logic')}. "
                         f"Confirming against it would compare with bytes the sets do not "
                         f"describe. Run `build` first.")
    values_by_locale_by_source: dict = {}
    for source, block in sorted((manifest.get("sources") or {}).items()):
        ...
        if corpus_digest(app, corpus_files(app, source)) != block.get("corpus_digest"):
            raise CanonError(f"{source}: the installed corpus is not the pinned one. Run `build` "
                             f"first.")
        by_locale: dict = {}
        for _unit, locale, _key, _field, value in extractor(app):
            by_locale.setdefault(locale, set()).add(normalize(value))
        values_by_locale_by_source[source] = by_locale
    asked = {text for (_source, _locale, text) in load_presence_ledger()} | set(texts)
    rows = presence_rows(values_by_locale_by_source, asked)
    write_ledger_presence(rows)
    manifest["ledger_presence_entries"] = presence_counts(rows)
    manifest["artifacts"] = artifact_digests()
    with open(MANIFEST_PATH, "w", encoding="utf-8") as handle:
        json.dump(manifest, handle, ensure_ascii=False, indent=2, sort_keys=True)
        handle.write("\n")
...
def artifact_digests() -> dict:
    """sha256 of every committed index, absence and ledger file, keyed by path relative to docs/canon."""
    out = {}
    for directory in (INDEX_DIR, ABSENCE_DIR, LEDGER_DIR):
...
def verify_derived_counts(manifest: dict) -> list:
    """`translated_en_values` and every `folded_entries` count, against the files they describe.

    #992. `build` wrote both and nothing read them back, so each could be edited with its file
    and the pair re-pinned, and nothing would disagree. The same trade `verify_absence_counts`
    makes: a forgery needs three consistent edits instead of two. Every folded set on disk, and
    every locale a source lists, needs a count -- a check that reads only the counts declared is
    deleted by deleting the count.
    """
    problems = []
    ...
    if not os.path.isfile(translated_path()):
        ...
        try:
            ...
            problems.append(f"absence/translated.en.u32: {exc}")
        else:
            if found != declared:
                problems.append(
                    f"absence/translated.en.u32 holds {found} entries and MANIFEST.json declares "
                    f"{declared}. The set decides which English labels count as translated.")
    sources = manifest.get("sources") or {}
    required = set()
    if os.path.isdir(ABSENCE_DIR):
        for name in os.listdir(ABSENCE_DIR):
            match = re.fullmatch(r"([^.]+)\.([^.]+)\.folded\.u32", name)
            if match and match.group(1) in EXTRACTORS:
                required.add(match.groups())
    for source, block in sources.items():
        required.update((source, locale) for locale in (block.get("locales") or []))
    problems.extend(
        f"absence/{source}.{locale}.folded.u32 has no count in MANIFEST.json, so nothing checks "
        f"that it did not gain or lose entries."
        for source, locale in sorted(required)
        if locale not in ((sources.get(source) or {}).get("folded_entries") or {}))
    for source, block in sorted(sources.items()):
        for locale, count in sorted((block.get("folded_entries") or {}).items()):
            try:
                found = len(load_absence(source, locale, folded=True))
            except CanonError as exc:
                problems.append(f"absence/{source}.{locale}.folded.u32: {exc}")
                continue
            if found != count:
                problems.append(
                    f"absence/{source}.{locale}.folded.u32 holds {found} entries and MANIFEST.json "
                    f"declares {count}. The folded set decides what counts as a near miss.")
    return problems
...
def _cmd_absent(args) -> int:
    ...
    if absent:
        ...
        return 0
    verdict = presence(args.source, args.locale, args.text)
    if verdict == ABSENT:
        print(f"ABSENT from {args.source}/{args.locale}: its 32-bit prefix collides with a value "
              f"the corpus holds, and ledger/presence.tsv pins the string comparison that says so.")
        return 0
    if verdict == SHIPS:
        print(f"PRESENT in {args.source}/{args.locale} (ledger/presence.tsv pins the string "
              f"comparison)", file=sys.stderr)
        return 1
    print(f"PRESENT (or colliding) in {args.source}/{args.locale} "
          f"({entries} values pinned; collision chance {rate:.3e}). "
          f"`Scripts/logic_canon.py confirm` on a machine with Logic says which.", file=sys.stderr)
    return 1
...
def main(argv=None) -> int:
    ...
    confirm_cmd = sub.add_parser(
        "confirm", help="compare strings whose 32-bit prefix matches, and pin the answer (needs Logic)")
    confirm_cmd.add_argument("text", nargs="*")
    confirm_cmd.add_argument("--stdin", action="store_true", help="also read one string per line")
    confirm_cmd.set_defaults(func=_cmd_confirm)
