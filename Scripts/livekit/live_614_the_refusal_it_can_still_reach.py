...
def _control_bar_band():
    ...
    if not (isinstance(b, list) and len(b) == 4):
        return None, None
    # The identity travels with the rectangle. A band recorded only as (x, y, w, h) cannot show that
    # it moved onto something else — three rectangles over this window each produced a confident
    # wrong answer and all three receipts looked the same.
...
ev.visual("614/the-transport-is-undisturbed",
          ...
          subject=f'AXDescription={band_subject!r} (located at run time, not a written rectangle)',
