...
- [ ] AC-3: `computeFixPlan(checks:)` returns exactly the checks where `status != .pass` **AND** `blockedBy == nil` **AND** `severity ∈ {error, warning}` (i.e. `status ∈ {fail, warn, manual}`), ordered **`fail → warn → manual`** then declared order; `skipped`/`pass` (`info`) excluded; `manual` **included** (§4.3 Fix Plan, canonical).
...
| 6 | `test_t1v3_fix_plan_order_fail_then_warn_then_manual` | Unit | 3-tier order | fixture with `fail`,`warn`,`manual` present ⇒ `fix_plan` orders fail-ids, then warn-ids, then manual-ids; declared-order tiebreak within a tier |
...
| 10 | `test_t1v3_dependency_table_and_status_resolver` | Unit | AC-5 table + resolver | `blockedByDependencies["mcp.registration_target"] == "mcp.claude_code_registration"` etc.; `status(of:in:)` returns the cause's status from a checks array; missing id ⇒ `nil` |
| 11 | `test_t1v3_doctor_tool_allowlist_rejects_arbitrary_binary` | Unit | AC-6 fail-closed | `SetupDoctor.Runtime.production.runCommand("/opt/homebrew/bin/LogicProMCP", ["--version"]) == nil` (no spawn); an arbitrary `/tmp/foo` ⇒ `nil` |
...
5. Add `static let blockedByDependencies: [String: String]` mapping derived-id → cause-id: `"mcp.registration_target": "mcp.claude_code_registration"`, `"logic.version_support": "logic.installation"`; plus `logic.blocking_dialog` (running-first two-cause precedence — handled in N4's body, T3; the table documents its cause set). Add `static func status(of id: String, in checks: [Check]) -> CheckStatus?`.
6. Add `static func computeFixPlan(_ checks: [Check]) -> [String]`: filter (`status != .pass && blockedBy == nil && severity != .info`), then stable-sort by tier rank (`fail=0, warn=1, manual=2`) preserving declared order within a tier, map to ids. **This is the ordered array — T8 renders it, does not re-sort.**
