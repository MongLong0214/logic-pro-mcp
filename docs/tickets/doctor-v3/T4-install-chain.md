...
- [ ] AC-4 (never execute, C4/NG7/D10): `lipo`/`file`/`strings` run via `BoundedProcessRunner` (≤1.5s) and are metadata-only. **No candidate is ever handed to `runCommand` as an executable** — enforced by T1's `DoctorTool` allowlist (LogicProMCP is not a tool ⇒ fail-closed nil).
