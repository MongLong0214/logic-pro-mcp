...
- [ ] AC-4 (N4 gated + precedence, OQ-3): `logic.blocking_dialog` gate = Logic running **AND** Accessibility granted. `warn` if a blocking dialog present; `pass` if none; `skipped` if gated with **running-first** `blocked_by`: Logic not running → `logic.application_state`; else Accessibility denied → `permissions.accessibility`. evidence (when present) `{dialog_present:"true",dialog_title,role,buttons,recovery_action}`; remediation `manual`.
...
| 13 | `test_t3_n4_skipped_bb_application_state_when_not_running` | Unit | N4 running-first (OQ-3) | Logic not running (AX denied too) ⇒ `.skipped` `blocked_by=="logic.application_state"` |
...
5. `logicBlockingDialogCheck` (N4): gate on `runtime.logicProRunning()` and `permissions.accessibility` (read via `status(of:"permissions.accessibility",in:checks) == .pass`). If gated ⇒ `skipped` with running-first `blockedBy`. Else call `runtime.blockingDialogProbe()`; `warn` if non-nil (populate evidence, `recovery_action` from `BlockingDialogInfo.recoveryAction`), else `pass`. remediation `manual`.
