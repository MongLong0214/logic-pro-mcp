...
## 3. Verification
Grep each doc for version strings, test counts, E2E counts, channel-routing claims, and the mixer "send" wording; cross-check each against the actual code/`swift test` output. A reviewer (safety review or independent review) diffs docs vs code claims.
