...
- [ ] AC-3 (PRD AC-2.2/2.3): `insert_verified {insert:0, plugin:"Gain", mode:"duplicate_applyback", project_expected_path}` → State A with post-insert readback; follow-up `get_inventory` shows Gain occupied at insert 0 (`plugin_id: logic.stock.effect.gain`) + trailing empty row.
