...
- ~~**Editor (graphical) view**: parameters are drawn on a custom `AXGroup` (desc=`EQ`) canvas — no per-band `AXSlider`/`AXValueIndicator` exposed. `entire contents` of the window filtered for slider/value roles = empty.~~ **REFUTED 2026-09-04 (#767).** The reading was the instrument, not the plug-in: `entire contents` returns an empty list without raising for every application on this host. A manual descent of the same `AXDialog`, seconds apart in the same process, finds **70 elements and 26 `AXSlider`s** (25 distinct names — two are called `Gain`) — `Low Cut Frequency/Order/Q`, `Low Shelf F/G/Q`, `Peak 1`–`Peak 4` F/G/Q, `High Shelf F/G/Q`, `High Cut F/Order/Q`, `Gain` — each with a raw `AXValue` and a rendered `AXValueDescription` (`«42.2 Hz»`, `«+2.2 dB»`, `«18 dB/Oct»`). They are also actuable, though not by setting: an `AXValue` write moves the control **one step toward** the requested value and returns success, so reaching a value is a bounded convergence loop with a readback between iterations (measured: six writes of `before + 20` walked 262 → 268; six writes of `before` restored it exactly). **This is specific to Channel EQ.** Two other editors opened in the same session expose zero sliders — ChromaVerb (21 elements) and the Studio Grand instrument (29) — so the sentence above is wrong about the plug-in this document is named after and right about its neighbours. See `docs/observations/2026-09-04-channel-eq-parameters-are-readable.json`.
