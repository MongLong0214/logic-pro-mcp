...
| #803 | OPEN | The live qualification gate's known-failure table assumes an English Logic. `realReleaseBinaryExecutesEveryOperationWithIndependentReadback` requires the observed live-gate failures to match `knownLiveGateFailures` EXACTLY, and that set was measured against an English Logic — so switching the UI language turns `main` red and blocks every branch. Measured 2026-09-07, same commit and same binary with one variable: `origin/main` in ja-JP FAILS at `QualificationRunnerTests.swift:1598`, `origin/main` in en-US PASSES in 45.729s, and `fix/774` in ja-JP fails at the same line with none of its code implicated. The exactness is right — `knownLiveGateFailureSetRejectsAKnownFailureStartingToPass` exists to catch a failure that quietly stops failing — but a set stated without the condition it was measured under cannot be satisfied in two languages, and the harness that must run in Japanese is what leaves the machine unable to satisfy it. NOT established: which operation's failure set moves, because the assertion reports only a boolean; and whether the Library panel's state is a second axis, which this run did not vary |
