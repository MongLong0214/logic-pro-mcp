...
| #804 | OPEN | A wall-clock assertion measures the CI runner, not the bounded refusal it names. `switchThatNeverChangesStructureRefusesWithinDeadlineAndLeavesEntryView` requires `Date().timeIntervalSince(started) < 0.75`, and on a loaded GitHub runner that is not about the code: the same test failed twice on a pull request changing NO Swift at all, at 0.771 and 0.778 seconds — both ~3% over — while passing locally across 4431 tests and passing on CI for another branch in the same window. Its own comment says the operation it bounds was measured at 28-46 ms with a 25 ms poll, so 0.75 s is already ~16x and the overrun is scheduling. Raising the constant would move the grader without changing what is measured; the intended property — that the refusal is bounded rather than hanging — is a property of the code (polls performed, the deadline the writer enforces) and not of the machine. NOT claimed: that this is the only such assertion; other `Date().timeIntervalSince` sites were not audited |
