...
| #757 | OPEN | the sort's settle witness takes its first read with no delay and accepts two equal consecutive reads, so a sort landing slower than ~75 ms has its own PRE-sort order reported as settled. Measured over 5 live drives: 4 State A idle, 1 State B under a full suite, whose receipt carried sorted `after_tracks` beside an unsorted `after_order` because the two came from different reads. Conservative — it under-reports a real sort rather than awarding State A to something unproven. Split out of #746 |
