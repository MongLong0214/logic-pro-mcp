...
| ADR-016 | #304 | OPEN | measured 2026-09-02 on live Logic 12.3 ko-KR: `편집 > 템포 > 템포 목록 보기` opens a fully-classified AXTable (one AXRow, three AXCells per event). The cell AXGroups expose whole position / numeric tempo / SMPTE descriptions, and `항목 수` independently reports the event count. The implementation is an internal, unwired reader plus a converging existing-event writer whose loop is not yet live-qualified; it is not a runtime capability. Creating/deleting events, six disabled region-tempo commands, and the disabled Smart Tempo editor remain UNMEASURED and refused. |
