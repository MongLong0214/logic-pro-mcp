...
| #747 | OPEN | `project.save_as` was dead on a Korean Logic: the menu item, the panel title and the two organise-as radios were all English literals, and Command-Shift-G opens no Go-to-Folder sheet on this build. Measured and fixed on `fix/747-save-as-korean-label`, proven by a live save; closes when that lands |
