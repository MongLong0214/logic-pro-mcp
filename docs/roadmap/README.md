...
## State — measured 2026-09-05 at `fadac62d`

```
v3.15.0 published · ADR-019 landed · 22 open issues, 2 closing with this change
```
...
| #769 | closed | closed 2026-09-05. `operations_driven` counted `tools/call` only, so a harness driving the product through `resources/read` scored zero and could never satisfy `is_clean` — the clause's own docstring says it rules out a run that never touched the product. Resource reads count now, named by URI. Proved live at `fadac62d`: `live_291_output_slot_is_read` came back 9/9 with `operations_driven: 7`, where it had always been 0. |
