...
| ADR-016 | #304 | OPEN | measured 2026-09-02 on live Logic 12.3 ko-KR: `편집 > 템포 > 템포 목록 보기` opens a fully-classified AXTable (one AXRow, three AXCells per event). The cell AXGroups expose whole position / numeric tempo / SMPTE descriptions, and `항목 수` independently reports the event count. Existing-event tempo writes are a verified one-BPM-at-a-time convergence loop with fresh row resolution and rollback; creating/deleting events, six disabled region-tempo commands, and the disabled Smart Tempo editor remain UNMEASURED and refused. |
