...
| ADR-011 | #299 | OPEN | behind #292, and "no write attempted anywhere" no longer holds. The *native* editor census stands as measured 2026-08-30: 22 sliders (11 settable) on one Compressor instance and 20 (10 settable) on another, naming exactly one parameter (`Threshold`) on both — also the only one `set_param_verified` supports, consistent with step 9 matching on `AXDescription`, though that link is inferred from the code path rather than measured. Writes HAVE been attempted, on the Controls view, and both non-checkbox roles are walls: AXSlider value and increment each reported success on 2026-09-02 and moved nothing, and the popups open but expose only the current choice or several identically-titled options, so no choice is nameable. The checkbox path is the one that works. The refusal reason for popups said "selection has not been measured", which was true when it was written on 2026-09-02 and stale two days later; it now states the wall |
...
| #766 | OPEN | `inferTrackType` answers `audio` for every track. Measured 2026-09-04: three tracks created back to back — software instrument, audio, drummer — returned one `observed_track_type` between them, and the aggregate the classifier reads carries identical type tokens on all 7 headers, because the Input Monitoring help text present on every header names an audio track and a software instrument track in the same sentence. Whichever branch runs first wins, and every order is constant, so a swap does not repair it. `SemanticOracleTable` calls this field the per-op discriminator; three of the four `create_*` oracles cannot pass against live Logic |
| #767 | OPEN | AppleScript `entire contents` returns an empty list, without raising, for every application on this host — measured 2026-09-04 at 0 for a Logic window where a manual descent of the same window finds 464 elements, and 0 for nine other applications. One `Sources` site uses it (`region.select_last`, in `RoutingTable` and not in `OperationRegistry`, so nothing reaches it), two `#291` harnesses build their independent witness from it and fail closed rather than falsely, and two documented AX-opacity conclusions cite an empty result from it as their reading. The same harnesses also select their window by an English literal. Evidence from 2026-08-23 has one of them clean, so this is a regression and the cause is still open |
