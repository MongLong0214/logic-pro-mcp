...
| #736 | closed | external report — duplicate virtual MIDI endpoints. PR #738 landed the ownership-conflict path (`MIDIEngine.swift:488`, `MIDIPortManager.swift:370`) after review refuted the census/create approach. Verified on main by the reporter, who ran three tests: an owner instance publishes all four ports; a second instance on the same machine declines rather than publishing a twin, reporting `has_foreign_endpoint` and `endpoint_count: 2`; and the degraded instance's own `midi.list_ports` / `midi.send_cc` answer State C naming the contended port |
