...
Public commands: `set_volume`, `set_pan`, `set_master_volume`, `set_plugin_param`, `insert_plugin`, `bank`.
...
`bank` moves the Mackie Control fader bank by eight strips per step: `{ direction: "left" | "right", count?: Int }` (`count` 1–31, default 1), MCU only. Its readback is the MCU LCD upper row, the 56-character line that names the eight visible strips. State A (`verify_source: mcu_lcd_upper_row`, with `window_before`, `window_after`, `strips` and `bank_presses_sent`) requires a fresh upper-row write after the press, a row that then holds still for one more poll, AND a row that differs from the pre-press snapshot; the press having been sent never counts. State A says the visible window changed; it does not confirm how many banks it moved, and a row that changes for another reason in the same interval (a rename, Logic re-banking on selection) reads the same. State B `noop_unobservable` means the row redrew unchanged (identical six-character names cannot confirm a move); State B `echo_timeout_<ms>ms` means no upper-row write arrived within the MCU echo timeout. State C `readback_unavailable` with `write_attempted: false` is returned before any byte is sent when the upper row has never been received on this server (no Mackie Control surface bound). `bank_bookkeeping_before` / `bank_bookkeeping_after` carry the server's own bank counter, which Logic's re-banking on track selection can leave stale; the reply exposes that drift, it does not correct it.
