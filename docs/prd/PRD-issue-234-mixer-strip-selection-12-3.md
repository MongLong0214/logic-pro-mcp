...
- [ ] AC-4.1: Given a fixture window `subrole=AXDialog` with the normative plugin-window chrome (D4: `kAXCloseButtonAttribute` present + bypass checkbox + compare checkbox; the live window additionally shows toolbar/link/view chrome, kept as evidence, not as conjuncts), `blockingDialogInfo`/`dialogPresent` classify it non-blocking — asserted through these public surfaces.
...
- [ ] AC-4.4: A window matching *some but not all* of the plugin-chrome signature stays blocking (fail-closed conservatism) — variants: bypass without compare, compare without bypass, both checkboxes but **no close-button attribute**, right labels on non-checkbox roles.
