...
**Version**: 0.3 (post boomer PRD R2 — 3 findings folded)
...
- G6 [safety gate] **Zero external-behavior change, with ONE documented exception** (boomer PRD-R2 #1): the `logic://tracks` + `logic://tracks/{index}` honesty correction — `volume`/`pan`/`automationMode` currently report FABRICATED `0.0`/`0.0`/`.off`; the fix makes them report the REAL track-header values (keys preserved, values become honest). This is the only intended observable change. Proven by: `swift test --no-parallel` green after each ticket + **golden byte-snapshots** (tool descriptors, unknown-command/invalid-param errors, representative HC envelopes, route mappings, resource URIs) captured pre-refactor and diffed post — diff must be `0` EXCEPT a constrained allowlist covering only those 3 track-resource fields — + strict live E2E parity + a broad live-surface baseline↔post diff (§5).
- G7 Ship **v3.8.0** (minor: substantial internal hardening + test integrity + docs + security fixes; no public tool/resource/template surface change — the `extractTrackState` correction PRESERVES all keys and only makes values honest, so it is a minor-appropriate honesty fix, NOT a schema/migration change; documented in CHANGELOG + API.md; bundles merged #234).
...
| E9 | `logic://tracks` honesty fix changes observed output (G6 exception) | golden-snapshot allowlist limited to volume/pan/automationMode; live-verify real values on 12.3; CHANGELOG + API.md document the correction; keys preserved so no client-parse break |
