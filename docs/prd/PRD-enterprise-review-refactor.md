...
**Version**: 0.4 (post independent review PRD R3 — value-only extractTrackState, no type change)
**Author**: Product planning and implementation team
**Date**: 2026-07-05
**Status**: Approved (independent review CONVERGED R4, 2026-07-05)
**Size**: XL
...
- **NG9 [new] Cooperative-pool blocking sweep** (setTempo 370ms / runLiveScan 50s / AppleScript-on-pool) — MEDIUM risk, wide, NOT correctness (mitigated: mutation-gate serializes, reads don't sleep, stdio+deadline off-pool). Concurrency audit + independent review both flag as risky-wide. DEFER (follow-up issue).
- **NG10 [new] CGEvent async-sleep** (postShortcutSequence) — naive Task.sleep introduces actor reentrancy → concurrent goto_position keystroke interleave (independent review PRD #2). DEFER unless done as a serialized key-worker (out of scope this sweep).
...
- **NG12 [new] LogicProServer deadline-race dedup (round-1 #10)** — load-bearing #112/#199 concurrency backstop; independent review "own commit, last." DEFER.
...
| D2 | Tests (WS8) after source merges | one sequential owner per test file; WS8 sees final assertions (incl. Phase-1-touched) |
...
| D4 | Dead-assertion: per-case transform + LEDGER + flip-test on safety-critical | independent review #3: wrong optional transform can pass while asserting wrong nil semantics |
| D5 | Golden byte-snapshots pre/post for wire-sensitive refactors | independent review #4: tests can pass while strings/envelopes/routes drift; also = the broad behavior-preservation E2E the user asked for |
