...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-3.0.3-blue.svg?style=flat-square" />
</p>
...
```bash
curl -fsSL https://raw.githubusercontent.com/MongLong0214/logic-pro-mcp/v3.0.3/Scripts/install.sh -o install.sh
...
bash <(curl -fsSL https://raw.githubusercontent.com/MongLong0214/logic-pro-mcp/v3.0.3/Scripts/install.sh)
```
...
**v3.0.3** (2026-04-20) — Production-ready, adhoc-signed pre-release.
...
- **747 unit + integration tests passing** on the v3.0.3 branch (`swift test --skip testLogicProServerStartCoversDefaultPortAndPollerLifecyclePaths --skip testStatePollerStartStopLifecycle` — 2 timer-driven lifecycle tests are deterministic-only under load-bearing CI, not product code)
...
- **v3.0.3 AX-native control surface**: primary GUI touchpoints (track selection, plugin Setting popup) now prefer Apple AX actions (AXPress, AXShowMenu, AXSelectedChildren) with CGEvent only as a last-resort fallback — reduces fragility under Logic UI updates
