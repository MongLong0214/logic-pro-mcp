...
enum SemanticOracleTable {
    ...
    static let structurallyUnverifiedMutatingOperationIDs: [OperationID: String] = [
        .systemSetupControlSurface:
            // Arrived on main after this inventory closed (#891). NOT back-dated into one of the
            // phase sets: those record what was pinned when, and putting a later operation into
            // one would falsify that record -- the same reasoning that kept `region.move` out of
            // `phaseB4MutatingOperationIDs`.
            //
            // The exclusion is about the TARGET, not our routing. What this operation changes
            // lives in `~/Library/Preferences/com.apple.logic.pro.cs` and in Logic's Control
            // Surface Setup window; no `logic://` resource publishes either, so there is nothing
            // for an independent readback to read. Its own evidence is MCU feedback arriving
            // after the write, which is the operation vouching for itself. A State-A fixture here
            // would pin a contract no reader can check.
            "no independent readback exists — the installed surface and its two MIDI port "
