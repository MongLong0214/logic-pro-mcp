...
enum AtlasQualification {
    ...
    //
    // It is INERT in every documented invocation: `QualificationRunner` reaches it only when the
    // atlas is armed, which needs `LOGIC_MCP_ADR007_SELECTOR_ATLAS=1` AND `LOGIC_MCP_ATLAS_BASELINES`,
    // and nothing in this repository sets either. Arming it is a separate decision -- the case it
    // emits carries no evidence files, which the verifier's cardinality rule refuses, so a bundle
    // built with the atlas armed would be rejected rather than admitted. That is the safe
    // direction and it is also why arming it is not a flag flip.
